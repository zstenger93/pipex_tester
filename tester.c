...
int main(int argc, char *argv[]) {
    if (argc != 5) {
        fprintf(stderr, "Usage: %s Makefile \"cat\" \"cat\" outfile\n", argv[0]);
        return 1;
    }
    ...
    char pipex_path[MAX_LENGTH] = "../pipex";
    char command[MAX_LENGTH];
    sprintf(command, "%s %s \"%s\" \"%s\" %s", pipex_path, makefile, command1, command2, outfile);
    system(command);

    char expected_output[MAX_LENGTH];
    sprintf(expected_output, "%s.expected", outfile);
    char expected_command[MAX_LENGTH];
    sprintf(expected_command, "< %s %s | %s > %s", makefile, command1, command2, expected_output);
    ...
    // printf("%d , %d\n", x, i);
    // printf("%d , %d\n", k, j);
    if (strlen(argv[2]) == i || strlen(argv[3]) == j)
    {
        
        return (0);
    }
    else if (strncmp(argv[2], "ls", 2) == 0 || strncmp(argv[3], "ls", 2) == 0)
    {
